...
!!! info "Prerequisites"
    This document assumes familiarity with the concepts of side-channel attacks, speculative execution, and [Speculation Contracts and Model-based Relational Testing (MRT)](../contracts.md).
...
The `DataGenerator` class (in `data_generator.py`) creates these inputs and returns them as `InputData` objects. See [binary formats](binary-formats.md#revizor-data-binary-format-rdbf) for the structure of input data.
...
Revizor supports multiple model backends: [Unicorn](model-unicorn.md) (CPU emulator) and [DynamoRIO](model-dr.md) (dynamic instrumentation). Both implement the same interface.
...
This approach focuses on information leakage rather than exact trace values, and it essentially implements a non-interference check (see [Theoretical Foundations](../contracts.md)).

The `Analyser` class (in `analyser.py`) implements this logic in its `filter_violations()` method.

## 7. Post-violation Analysis

When Revizor detects a potential violation, it runs additional tests to filter out false positives. These tests modify execution parameters and verify the violation still occurs. See [post-violation tests](arch-mini.md) for details.

If the violation survives all filters, Revizor reports it to the user and saves reproduction artifacts. The user can then use [minimization tools](../user/minimization.md) to simplify the test case and identify the root cause.
