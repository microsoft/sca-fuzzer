...
_DRRUN_CMD: Final[str] = "~/.local/dynamorio/drrun -c ~/.local/dynamorio/libdr_model.so " \
    "--trace-output " + _TRACE_FILE + " {flags} -- {binary} {args}"
...
class DynamoRIOModel(Model):
    ...
    def _clean_trace_files(self) -> None:
        ...
        with open(_TRACE_FILE, 'wb') as tf:
            tf.truncate()
        with open(_DBG_TRACE_FILE, 'wb') as dbg_tf:
            dbg_tf.truncate()
