...
class EquivalenceAnalyser(Analyser):
    ...
    def filter_violations(self,
        ...
        :return A list of input IDs where ctraces disagree with htraces and a list of inputs that
            require retries
        """
        equivalence_classes: EquivalenceClassMap = self.build_equivalence_classes(
    ...
    def build_equivalence_classes(self,
                                  ...
                                  ctraces: List[CTrace],
                                  htraces: List[HTrace],
                                  stats=False) -> EquivalenceClassMap:
        """
        ...
        if stats:
            ...
            for eq_cls in equivalence_classes.values():
                if len(eq_cls.inputs) == 1:
                    num_single_entry += 1
            STAT.single_entry_classes += num_single_entry
            STAT.eff_classes += len(equivalence_classes) - num_single_entry
