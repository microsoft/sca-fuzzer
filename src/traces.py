...
REG_ID_TO_NAME = {0: "rax", 1: "rbx", 2: "rcx", 3: "rdx", 4: "rsi", 5: "rdi"}
...
class HTrace:
    ...
    def _full_arch_str(self, line_prefix: str) -> str:
        ...
        s += "["
        s += f"{REG_ID_TO_NAME[0]}: 0x{self._raw[0]['trace']:x}, "
        s += f"{REG_ID_TO_NAME[1]}: 0x{self._raw[0]['pfc0']:x}, "
        s += f"{REG_ID_TO_NAME[2]}: 0x{self._raw[0]['pfc1']:x}, "
        s += f"{REG_ID_TO_NAME[3]}: 0x{self._raw[0]['pfc2']:x}, "
        s += f"{REG_ID_TO_NAME[4]}: 0x{self._raw[0]['pfc3']:x}, "
        s += f"{REG_ID_TO_NAME[5]}: 0x{self._raw[0]['pfc4']:x}"
        s += "]"
        return s
